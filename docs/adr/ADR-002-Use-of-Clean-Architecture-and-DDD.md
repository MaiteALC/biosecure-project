...
The code will be divided into concentric layers, where the dependencies point only inwards (to the domain layer). Thus, we ensure that the core is protected and isolated from the external world, without the need to know about anything outside. Furthermore, the code follows DDD concepts like ubiquitous language, aggregates, value objects, aggregate invariants. In this way the code base gains more domain alignment, especially regarding technical terms.
